...
For the features “gill-spacing", “cap-surface", “ring-type" and “gill-attachment" the use of imputation was attempted, performing a grafical analysis to evaluate how frequently the entries occurred for each feature. The possibility of mean imputation was considered but the high amount of missing entries led to dropping them.\\  

The features “cap-shape", “cap-color", “gill-color", “stem-color", “habitat" and “season" had their values represented as letters and - since most algorithms only support numerical values, encoding was needed.\\ 

Simply mapping the letters as numbers could mislead some algorithms to see the values as a scale, which is not true in this case of colors, seasons, shapes and habitats, so the one-hot encoding was chosen as the appropriate approach.\\ 

However, applying it to all 6 features increased the amount of features to 51 (with an increase of 30 from the original 21, more than doubling it). This amount will, however, be later reduced to simplify computations and save resourses.\\
...
The features “season\_a\_encoded" and “season\_u\_encoded" showed a correlation of over 0.75, so one of the features ("season\_a\_encoded") was discarded.\\

This concluded the data understanding phase.\\

The next step was the modeling of the data for the training of the machine learning algorithm. This consisted in the splitting of the data between training and test sets.\\

After the splitting the decision tree classifier was considered as a possible algorithm and, after the implementation, an accuracy score to evaluate the model.\\

In addition to the accuracy score, a graphical representation of the accuracy compared to the depht of the decision tree was done for analysis purposes. Also, a feature importance was attempted to see if all all the feature selected were relevant in the modeling of the algorithm.\\
...
Lastly, AutoML was used to evaluate the best model and hyperparameters for the Secondary mushroom dataset.\\
